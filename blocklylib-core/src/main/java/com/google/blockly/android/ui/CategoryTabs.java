...
import com.google.blockly.model.ToolboxModel;
...
public class CategoryTabs extends RecyclerView {
    ...
    private void initToolboxModel() {
        List<Integer> categoryColors = new ArrayList<>();
        categoryColors.add(R.color.blockly_start_color);
        categoryColors.add(R.color.blockly_movement_color);
        categoryColors.add(R.color.blockly_light_and_sound_color);
        categoryColors.add(R.color.blockly_condition_color);
        categoryColors.add(R.color.blockly_detect_color);
        categoryColors.add(R.color.blockly_logic_color);
        categoryColors.add(R.color.blockly_loop_color);
        categoryColors.add(R.color.blockly_math_color);
        categoryColors.add(R.color.blockly_variable_color);
        categoryColors.add(R.color.blockly_fun_color);

        List<Integer> activeIcons = new ArrayList<>();
        activeIcons.add(R.drawable.start);
        activeIcons.add(R.drawable.movement);
        activeIcons.add(R.drawable.light);
        activeIcons.add(R.drawable.condition);
        activeIcons.add(R.drawable.detect);
        activeIcons.add(R.drawable.logic);
        activeIcons.add(R.drawable.loop);
        activeIcons.add(R.drawable.math);
        activeIcons.add(R.drawable.variable);
        activeIcons.add(R.drawable.func);

        List<Integer> passiveIcons = new ArrayList<>();
        passiveIcons.add(R.drawable.start_w);
        passiveIcons.add(R.drawable.movement_w);
        passiveIcons.add(R.drawable.light_w);
        passiveIcons.add(R.drawable.condition_w);
        passiveIcons.add(R.drawable.detect_w);
        passiveIcons.add(R.drawable.logic_w);
        passiveIcons.add(R.drawable.loop_w);
        passiveIcons.add(R.drawable.math_w);
        passiveIcons.add(R.drawable.variable_w);
        passiveIcons.add(R.drawable.func_w);

        toolboxModel = new ToolboxModel(categoryColors, activeIcons, passiveIcons);
    }
    ...
    public void setSelectedCategory(@Nullable BlocklyCategory category) {
        ...
        if (mCurrentCategory != null) {
            ...
            if (vh != null && mLabelAdapter != null) {  // Tab might not be rendered or visible yet.
                ...
                mLabelAdapter.onSelectionChanged(
                        vh.mLabel, vh.mCategory, vh.getAdapterPosition(), false, toolboxModel);
            }
        }
        ...
        if (mCurrentCategory != null && mLabelAdapter != null) {
            ...
            if (vh != null) {  // Tab might not be rendered or visible yet.
                // Update style. Don't use notifyItemChanged(..), due to a resulting UI flash.
                mLabelAdapter.onSelectionChanged(
                        vh.mLabel, vh.mCategory, vh.getAdapterPosition(), true, toolboxModel);
            }
        }
    ...
    private class CategoryAdapter extends RecyclerView.Adapter<TabLabelHolder> {
        ...
        public void onBindViewHolder(TabLabelHolder holder, int tabPosition) {
            ...
            mLabelAdapter.onSelectionChanged(holder.mLabel, category, tabPosition, isSelected, toolboxModel);
    ...
    protected class DefaultTabsAdapter extends CategoryTabs.LabelAdapter {
        ...
        public View onCreateLabel() {
            return (LinearLayout) LayoutInflater.from(getContext())
        ...
        public void onBindLabel(View labelView, BlocklyCategory category, int position) {
            ...
            TextView tvCategory = labelView.findViewById(R.id.toolbox_tab_tv);
            View viewLeft = labelView.findViewById(R.id.toolbox_tab_view);
            ImageView icon = labelView.findViewById(R.id.toolbox_tab_image);
            tvCategory.setText(labelText);
            // TODO: 21.11.2018
            tvCategory.setTextColor(ContextCompat.getColor(tvCategory.getContext(), toolboxModel.getActiveColors().get(position)));
            viewLeft.setBackgroundColor(ContextCompat.getColor(tvCategory.getContext(), toolboxModel.getActiveColors().get(position)));
            icon.setImageResource(toolboxModel.getActiveIcons().get(position));
        }
    }

    protected abstract static class LabelAdapter {
        private int counter;
        ...
        public void onSelectionChanged(
                View labelView, BlocklyCategory category, int position, boolean isSelected, ToolboxModel toolboxModel) {

            if (counter < 10 && position < 0) {
                ++counter;
                return;
            }

            category.setColor(toolboxModel.getActiveColors().get(position));
            labelView.setSelected(isSelected);

            TextView tvCategory = labelView.findViewById(R.id.toolbox_tab_tv);
            ImageView icon = labelView.findViewById(R.id.toolbox_tab_image);
            if (isSelected) {
                labelView.setBackgroundColor(ContextCompat.getColor(tvCategory.getContext(), toolboxModel.getActiveColors().get(position)));
                tvCategory.setTextColor(ContextCompat.getColor(tvCategory.getContext(), R.color.white));
                icon.setImageResource(toolboxModel.getPassiveIcons().get(position));
            } else {
                labelView.setBackgroundColor(ContextCompat.getColor(tvCategory.getContext(), R.color.white));
                tvCategory.setTextColor(ContextCompat.getColor(tvCategory.getContext(), toolboxModel.getActiveColors().get(position)));
                icon.setImageResource(toolboxModel.getActiveIcons().get(position));
            }
